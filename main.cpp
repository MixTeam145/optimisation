...
int main()
{
	...
	std::cout << "Select optimization function:\n"
	...
	do {
		...
		case 2:
			...
			do {
				...
				std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
				if (coeff <= 0)
					std::cout << "Incorrect coefficient (must be > 0)\n> ";
			} while (coeff <= 0);
			std::cout << "Enter seed\n> ";
			int seed;
			do {
	...
	std::cout << "Enter cubic domain of the function (" << f->dim() << "-dimentional)\n";
	Vector min_point(f->dim()), max_point(f->dim());
	std::cout << "Min point\n> ";
	std::cin >> min_point;
	std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
	std::cout << "Max point\n> ";
	...
	std::cout << "Enter start point (" << f->dim() << "-dimentional)\n> ";
	Vector start_point(f->dim());
	std::cin >> start_point;
	std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
